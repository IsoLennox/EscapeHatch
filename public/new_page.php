...
if (isset($_POST['submit'])) {
  ...
  if (empty($errors)) {
    ...
    $query .= "  subject_id, user_id, menu_name, position, visible, content";
    $query .= ") VALUES (";
    $query .= "  {$subject_id}, {$user_id}, '{$menu_name}', {$position}, {$visible}, '{$content}'";
    $query .= ")";
    ...
    if ($result) {
      ...
      redirect_to("user.php?subject=" . urlencode($current_subject["id"]));
    } else {
      // Failure
...
-->
    ...
    <br />
    <a href="user.php?subject=<?php echo urlencode($current_subject["id"]); ?>">Cancel</a>
  </div>
</div>
